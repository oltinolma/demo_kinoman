...
const TaxonomyField = (props) => {
    return (
        <Form.Field required>
            ...
            <TagInpInput placeholder={ props.parent } sendList={ props.sendList } />
        </Form.Field>
    )
}
...
class Taxanomy extends React.Component {
    ...
    render (props){
        ...
        return (
            <Form>
                {taxList.map(item => 
                    <TaxonomyField key={item.id} parent={item.name} taxonomy={item.value} sendList={this.props.sendList} />    
                )}
            </Form>
        )
    }
